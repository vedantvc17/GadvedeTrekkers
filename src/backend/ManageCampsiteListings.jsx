    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.45)", zIndex: 9000, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
      <div style={{ background: "#fff", borderRadius: 14, width: "100%", maxWidth: 680, maxHeight: "90vh", overflowY: "auto", boxShadow: "0 20px 60px rgba(0,0,0,0.25)" }}>
        <div style={{ padding: "18px 24px", borderBottom: "1px solid #e2e8f0", display: "flex", alignItems: "center", justifyContent: "space-between", position: "sticky", top: 0, background: "#fff", zIndex: 1 }}>
          <div style={{ fontWeight: 700, fontSize: 16, color: "#0c3a1e" }}>✏️ Edit Campsite — {listing.campsiteName}</div>
        ...
        <div style={{ padding: "24px" }}>
          <div className="row g-3">
            ...
            <div className="col-12">
              <label className="form-label fw-semibold">Campsite Name</label>
              <input className="form-control" value={form.campsiteName || ""} onChange={set("campsiteName")} />
            </div>
            <div className="col-md-6">
            ...
            <div className="col-md-4">
              <label className="form-label fw-semibold">Nearby Trek</label>
              <input className="form-control" value={form.nearbyTrek || ""} onChange={set("nearbyTrek")} />
            </div>
            <div className="col-md-4">
              <label className="form-label fw-semibold">Capacity (persons)</label>
              <input type="number" min="1" className="form-control" value={form.capacity || ""} onChange={set("capacity")} />
            </div>
            <div className="col-md-4">
              <label className="form-label fw-semibold">Price / Person (₹)</label>
              <input type="number" min="0" className="form-control" value={form.pricePerPerson || ""} onChange={set("pricePerPerson")} />
            </div>
            <div className="col-12">
            ...
            <div className="col-12">
              ...
              <textarea className="form-control" rows={2} placeholder="Internal notes..." value={form.adminNotes || ""} onChange={set("adminNotes")} />
            </div>
          </div>
        </div>
...
export default function ManageCampsiteListings() {
  ...
  const handleSaveEdit = async (updated) => {
    try {
      await updateListingSubmission("campsite", updated.id, updated);
      setListings(listings.map((l) => (l.id === updated.id ? updated : l)));
      setEditing(null);
      toast.success("Campsite listing updated successfully.");
    } catch (error) {
      toast.error(getErrorMessage(error, "Campsite listing could not be updated."));
    }
  };
  ...
  const handleDelete = async (id) => {
    ...
    if (!ok) return;
    try {
      await deleteListingSubmission("campsite", id);
  ...
  const setStatus = async (id, status) => {
    ...
    const ok = await confirm({
      ...
      message: `Are you sure you want to ${labels[status] || status.toLowerCase()} this campsite listing?`,
    ...
    try {
      await updateListingSubmission("campsite", id, { status });
      ...
      toast.success(`Campsite status updated to ${statusLabels[status] || status}.`);
    } catch (error) {
      toast.error(getErrorMessage(error, "Campsite status could not be updated."));
    }
  };

  const filtered = listings.filter((l) => {
    const q = search.toLowerCase();
    const matchSearch = !search || l.campsiteName?.toLowerCase().includes(q) || l.ownerName?.toLowerCase().includes(q) || l.location?.toLowerCase().includes(q);
  ...
  return (
    <div className="adm-page">
      ...
      <div className="adm-page-header">
        <h1 className="adm-page-title">⛺ Campsite Listings</h1>
      ...
      <div style={{ background: "#eff6ff", border: "1px solid #bfdbfe", borderRadius: 10, padding: "10px 16px", marginBottom: 16, fontSize: 13, color: "#1d4ed8" }}>
        🔒 These listings are submitted by campsite owners via the public form. They are visible to admin only until you change the status to <strong>Live</strong>.
      </div>
      ...
      {filtered.length === 0 ? (
        <div style={{ textAlign: "center", padding: "60px 20px", color: "#94a3b8" }}>
          ...
          <div style={{ fontWeight: 600 }}>{listings.length === 0 ? "No campsite listings yet" : "No listings match your search"}</div>
        </div>
      ) : (
              ...
              {filtered.map((l) => (
                <>
                  <tr key={l.id} style={{ cursor: "pointer" }} onClick={() => setExpandedId(expandedId === l.id ? null : l.id)}>
                    <td>
                      <div style={{ fontWeight: 600 }}>{l.campsiteName}</div>
                    ...
                    <td>{l.pricePerPerson ? `₹${Number(l.pricePerPerson).toLocaleString("en-IN")}` : "—"}</td>
                      ...
                      <td colSpan={8} style={{ padding: "16px 24px" }}>
                        <div className="row g-3">
                          ...
                          <div className="col-md-3">
                            ...
                            <p style={{ fontSize: 13, color: "#475569", marginTop: 4 }}>{Array.isArray(l.facilities) ? l.facilities.join(", ") : l.facilities || "—"}</p>
                          </div>
                          <div className="col-md-3">
